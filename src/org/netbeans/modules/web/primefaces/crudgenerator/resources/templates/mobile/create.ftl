...
<#else>
    ...
    <#if (doConfirmationDialogs && primeFacesVersion.compareTo("5.1.13") >= 0) >
                    <p:commandButton actionListener="${r"#{"}${managedBean}${r".saveNew}"}" value="${r"#{"}${bundle}.Save${r"}"}" update="display,:${entityName}ListPage:${entityName}ListForm:datalist,${messageUpdate}"<#if doRelationshipNavigation> action="${r"#{"}mobilePageController.navigateWithHistory('pm:${entityName}ListPage')${r"}"}"<#else> action="pm:${entityName}ListPage"</#if>>
